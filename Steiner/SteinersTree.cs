...
namespace Steiner
{
    public static class SteinersTree
    {
        public static List<KeyValuePair<Point, List<Point>>> BuildSteinersTree(List<Point> inputPoints)
        {
            ...
            foreach (List<Point> list in subsets)
            {
                steinersTreeForSubsets.Add(BuildSteinersTreeForSubset(list));
            }
            ...
            //return steinersTreeForSubsets[0].Select((p) => new KeyValuePair<Point, List<Point>>(p.First, p.Second)).ToList();
        }
        private static List<Pair<Point, List<Point>>> BuildSteinersTreeForSubset(List<Point> subset)
        {
            ...
            foreach (Deque<Pair<int, int>> deq in GetBuildingCombinations(subset))
            {
                double length = ContractionSet(deq, subset, out List<Pair<Point, List<Point>>> tmpResult);
                if (length < minLength)
                {
                    minLength = length;
                    result = tmpResult;
        ...
        private static double ContractionSet(Deque<Pair<int, int>> deq, List<Point> set, out List<Pair<Point, List<Point>>> result)
        {
            ...
            do
            {
                if (deq.Count == 0)
                {
                    ...
                    try
                    {
                        tree = ReverseCoures(new MyStack<Pair<Point[], Point?>>(st), tmpSubset);
                    }
                    catch (Exception)
                    {
                        tree = null;
        ...
        private static IEnumerable<Deque<Pair<int, int>>> GetBuildingCombinations(List<Point> subset)
        {
            ...
            for (int i = 0; i < subset.Count - 1; i++)
            {
                ...
                {
                    var tmpSubset = new List<Point>(subset);
                    ...
                    {
                        foreach (var deqq in GetBuildingCombinations(tmpSubset))
                        {
                            foreach (var p in deqq)
                            {
                                deq.PushBack(p);
                            }
                            yield return deq;
    ...
    internal class ListPointsEqulityComparer : EqualityComparer<List<Point>>
    {
        public override bool Equals(List<Point> x, List<Point> y)
        {
            return x.SequenceEqual(y);
        }

        public override int GetHashCode([DisallowNull] List<Point> obj)
        {
            return 0;
        }
    }
